...
class Neo4jDriver(GraphDatabaseDriver):
    ...
    def load_task(self, workflow, task):
        ...
        """
        with self._driver.session() as session:
            session.write_transaction(tx.create_task, workflow=workflow, task=task)
    ...
    def _get_task_hint_pairs(self, task_records):
        """Get a list of (task_record, hints) tuple pairs.
        ...
        return list(zip(trecords, hints))
...
def _reconstruct_hints(hint_records):
    """Reconstruct a task hint by its record retrieved from Neo4j.
