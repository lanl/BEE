...
* `exec_env_conf`: specifiying all configuration related to the execution environment.
	* a. `bee_vm`: Execution configuration related to `BEE-VM`;
		* a1. `cpu_core_per_socket`: number of CPU cores per socket;
		* a2. `cpu_thread_per_core`: number of CPU thread per core;
		* a3. `cpu_sockets`: number of CPU sockets;
		* a4. `ram_size`: RAM size (512M, 8G, etc.);
		* a5. `kvm_enabled`: whether KVM is enabled or not;
		* a6. `host_input_dir`: input file directory.
