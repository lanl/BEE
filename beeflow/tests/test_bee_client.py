...
def test_reexecute(mocker,tmpdir):
    wf_name = "success_workflow"
    wf_path = Path(str(tmpdir / "fake_wf"/ "fake.tar"))
    workdir = Path(str(tmpdir / "fake_wf"))

    mocker.patch("os.path.expanduser", return_value=str(workdir))
    mocker.patch("os.path.abspath", return_value=str(workdir))
    mocker.patch("os.path.exists", return_value = True)
    
    fake_cwl = mocker.Mock()
    fake_cwl.name = "1234/fake.cwl"

    fake_yaml = mocker.Mock()
    fake_yaml.name = "1234/fake.yml"

    archive = mocker.MagicMock()
    archive.getmembers.return_value = [fake_cwl, fake_yaml]
    archive.__enter__.return_value = archive

    mocker.patch("tarfile.open", return_value=archive)
    mocker.patch("yaml.load", return_value={"main_cwl": "fake.cwl", "yaml": "fake.yml"})

    mock_submit = mocker.patch("beeflow.client.bee_client.submit", return_value="123")
    
    result = bee_client.reexecute(wf_name, wf_path, workdir)
    assert result == "123"

def test_reexecute_tar_error(mocker,tmpdir):
    wf_name = "no_tar_wf"
    wf_path = Path(str(tmpdir / "no_exist"/ "fake.tar"))
    workdir = Path(str(tmpdir / "fake_wf"))

    mocker.patch("beeflow.client.bee_client._INTERACTIVE", False)
    mocker.patch("os.path.exists", return_value = False)
    
    with pytest.raises(bee_client.ClientError, match = "cannot be found"):
        bee_client.reexecute(wf_name, wf_path, workdir)

def test_reexecute_path_error(mocker,tmpdir):
    wf_name = "no_path_wf"
    wf_path = Path(str(tmpdir / "fake_wf"/ "fake.tar"))
    workdir = Path(str(tmpdir / "no_exist"))
    
    def correct_path(path):
        return str(path) == str(wf_path)

    mocker.patch("beeflow.client.bee_client._INTERACTIVE", False)
    mocker.patch("os.path.exists", side_effect=correct_path)

    with pytest.raises(bee_client.ClientError, match = "doesn't exist"):
        bee_client.reexecute(wf_name, wf_path, workdir)
