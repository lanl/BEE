...
class BeeCCLauncher(BeeTask):
    ...
    def __init__(self, task_id, beefile):
        ...
        self.__bee_cc_conf = beefile['exec_env_conf']['bee_cc']
        self.__docker_conf = beefile['docker_conf']
        self.__task_name = self.__task_conf['task_name']
        self.__task_id = task_id
        self.__reservation_id = self.__bee_cc_conf['reservation_id']

        # OS configuration
        self.__bee_cc_sgroup = '{}-{}-bee-os-security-group'.format(os.getlogin(), self.__task_name)
    ...
    def launch_stack(self):
        ...
        cmd = [ "openstack " +
                ...
                "--parameter bee_workers_count={} ".format(int(self.__bee_cc_conf['num_of_nodes']) - 1) +
                "--parameter key_name={} ".format(self.__ssh_key) +
                "--parameter reservation_id={} ".format(self.__reservation_id) +
                "--parameter security_group_name={} ".format(self.__bee_cc_sgroup) +
                ...
                if (sg.to_dict()['name'] == self.__bee_cc_sgroup):
                    ...
                    if (len(ip_list) == 2):
                        ...
                        master = BeeOS(self.__task_id, 
                                       ...
                                       ip_list[0],
                                       ip_list[1])
                        self.__bee_cc_list.insert(0, master)
                ...
                if (sg.to_dict()['name'] == self.__bee_cc_sgroup):
                    ...
                    if (len(ip_list) == 1):
                        ...
                        worker = BeeOS(self.__task_id, 
                                       ...
                                       self.__key_path,
                                       ip_list[0],
                                       self.__bee_cc_list[0].master_public_ip,
                                       self.__bee_cc_list[0])
                        self.__bee_cc_list.append(worker)
    ...
    def setup_sshkey(self):
        ...
        self.__bee_cc_list[0].copy_to_master(self.__ssh_dir + '/id_rsa', '/home/cc/.ssh/id_rsa')
        for i in range(1, len(self.__bee_cc_list)):
            self.__bee_cc_list[0].copy_to_worker('/home/cc/.ssh/id_rsa', '/home/cc/.ssh/id_rsa', self.__bee_cc_list[i])

    def setup_sshconfig(self):
        cprint('[' + self.__task_name + '] Copy ssh config into each node.', self.__output_color)
        self.__bee_cc_list[0].copy_to_master(self.__ssh_dir + '/config', '/home/cc/.ssh/config')
        for i in range(1, len(self.__bee_cc_list)):
            self.__bee_cc_list[0].copy_to_worker('/home/cc/.ssh/config', '/home/cc/.ssh/config', self.__bee_cc_list[i])

    def setup_hostname(self):
        cprint('[' + self.__task_name + '] Setup hostname for each node.', self.__output_color)
        for bee_cc in self.__bee_cc_list:
            bee_cc.set_hostname()

    def setup_hostfile(self):
        cprint('[' + self.__task_name + '] Setup /etc/hosts file for each node.', self.__output_color)
        # Setup hosts file
        for node1 in self.__bee_cc_list:
            for node2 in self.__bee_cc_list:
...
#
    ...
    def general_run(self):
        ...
        for run_conf in self.__task_conf['mpi_run']:
            ...
            master.copy_to_master(local_script_path, node_script_path)
            for bee_cc in self.__bee_cc_list:
                bee_cc.docker_copy_file(node_script_path, docker_script_path)
            # Generate hostfile and copy to container
            master.docker_make_hostfile(run_conf, self.__bee_cc_list, self.__tmp_dir)
